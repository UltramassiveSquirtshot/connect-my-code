...
function getSql() {
    const url = Netlify.env.get('DATABASE_URL') || Netlify.env.get('NETLIFY_DATABASE_URL');
    if (!url) return null;
    if (!sql) sql = neon(url);
    return sql;
}
...
export function isDbEnabled() {
    return Boolean(Netlify.env.get('DATABASE_URL') || Netlify.env.get('NETLIFY_DATABASE_URL'));
}
