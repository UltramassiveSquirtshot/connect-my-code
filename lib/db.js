// lib/db.js
// Supporta:
//   DATABASE_URL      → impostata manualmente
//   POSTGRES_URL      → impostata da Vercel Neon integration
//   NETLIFY_DATABASE_URL → legacy Netlify Neon
...
export async function saveTranscript({ videoId, youtubeUrl, transcript, summary }) {
    const db = getSql();
    if (!db) throw new Error('Database non configurato — imposta DATABASE_URL o POSTGRES_URL su Vercel.');
    await ensureSchema();
    const rows = await db`
